...
class PackageInstallerApp:
    ...
    def __init__(self, package_file: str = "packages.json", translations: Optional[Dict[str, str]] = None) -> None:
    ...
    def menu_instructions(self) -> str:
        return self.translations.get("menu_instructions", self.DEFAULT_TRANSLATIONS["menu_instructions"])

    @property
    def select_distro_text(self) -> str:
        return self.translations.get("select_distro", self.DEFAULT_TRANSLATIONS["select_distro"])

    @property
    def select_method_text(self) -> str:
        return self.translations.get("select_method", self.DEFAULT_TRANSLATIONS["select_method"])

    @property
    def select_category_text(self) -> str:
        return self.translations.get("select_category", self.DEFAULT_TRANSLATIONS["select_category"])

    @property
    def select_package_text(self) -> str:
        return self.translations.get("select_package", self.DEFAULT_TRANSLATIONS["select_package"])

    @property
    def installation_log_text(self) -> str:
        return self.translations.get("installation_log", self.DEFAULT_TRANSLATIONS["installation_log"])

    @property
    def uninstallation_log_text(self) -> str:
        return self.translations.get("uninstallation_log", self.DEFAULT_TRANSLATIONS["uninstallation_log"])

    @property
    def no_packages_install_text(self) -> str:
        return self.translations.get("no_packages_install", self.DEFAULT_TRANSLATIONS["no_packages_install"])

    @property
    def no_packages_uninstall_text(self) -> str:
        return self.translations.get("no_packages_uninstall", self.DEFAULT_TRANSLATIONS["no_packages_uninstall"])

    @property
    def install_complete_text(self) -> str:
        return self.translations.get("install_complete", self.DEFAULT_TRANSLATIONS["install_complete"])

    @property
    def category_install_complete_text(self) -> str:
        return self.translations.get("category_install_complete", self.DEFAULT_TRANSLATIONS["category_install_complete"])

    @property
    def category_uninstall_complete_text(self) -> str:
        return self.translations.get("category_uninstall_complete", self.DEFAULT_TRANSLATIONS["category_uninstall_complete"])

    @property
    def message_duration(self) -> int:
        try:
            return int(self.translations.get("message_duration", self.DEFAULT_TRANSLATIONS["message_duration"]))
        except ValueError:
            return 1500

    @property
    def install_button_text(self) -> str:
        return self.translations.get("install_button", self.DEFAULT_TRANSLATIONS["install_button"])

    @property
    def back_button_text(self) -> str:
        return self.translations.get("back_button", self.DEFAULT_TRANSLATIONS["back_button"])

    # -------------------------------------------------------------------------
    # Helper functions for JSON structure.
    # -------------------------------------------------------------------------
    def get_category_data(self, distro: str, method: str, category: str) -> Tuple[str, List[Any]]:
    ...
    def get_package_display_text(self, pkg: Optional[Union[str, Dict[str, Any]]]) -> str:
    ...
    def get_category_display_text(self, distro: str, method: str, category: str) -> str:
        ...
        total = len(packages)
        installed = sum(1 for pkg in packages if self.is_package_installed(method, self.get_pkg_name(pkg)))
    ...
    @staticmethod
    def get_install_command(method: str, package: str) -> str:
        if method == "APT":
    ...
    def draw_progress_bar(self, stdscr: curses.window, current: int, total: int,
                          row: int, col: int, bar_width: int) -> None:
    ...
    def show_temp_message(self, stdscr: curses.window, message: str, duration: Optional[int] = None) -> None:
    ...
    def list_installed_packages(self, stdscr: curses.window) -> None:
        ...
        if not self.selected_distro or not self.selected_method:
            self.popup_message(stdscr, "Please select a distribution and backend first.")
            return
        installed_list = []
        for category in self.package_data[self.selected_distro][self.selected_method]:
            _, packages = self.get_category_data(self.selected_distro, self.selected_method, category)
            for pkg in packages:
    ...
    def show_package_info(self, stdscr: curses.window, package: str, method: str) -> None:
        if method == "APT":
        ...
        info = (result.stdout.decode("utf-8", errors="replace") or
    ...
    def install_category_packages(self, stdscr: curses.window) -> None:
        ...
        selected_in_category = [pkg for pkg in packages if self.get_pkg_name(pkg) in self.selected_packages]
        if selected_in_category:
            to_install = selected_in_category
        else:
            to_install = [pkg for pkg in packages if not self.is_package_installed(self.selected_method, self.get_pkg_name(pkg))]
        if not to_install:
        ...
        for idx, pkg in enumerate(to_install, start=1):
            ...
            log_row = 5 + idx
            if self.is_package_installed(self.selected_method, pkg_name):
                stdscr.addstr(log_row, 2, f"Already installed {self.CATEGORY_MARKER_INSTALLED}: {pkg_name}",
            ...
            stdscr.refresh()
        stdscr.addstr(7 + total, 2, self.category_install_complete_text, curses.A_REVERSE)
        stdscr.refresh()
        stdscr.getch()

    def uninstall_category_packages(self, stdscr: curses.window) -> None:
        stdscr.clear()
        stdscr.addstr(1, 2, f"{self.back_button_text} {self.current_category}", curses.A_BOLD)
        stdscr.refresh()
        _, packages = self.get_category_data(self.selected_distro, self.selected_method, self.current_category)
        selected_in_category = [pkg for pkg in packages if self.get_pkg_name(pkg) in self.selected_packages]
        if selected_in_category:
            ...
            to_uninstall = [pkg for pkg in packages if self.is_package_installed(self.selected_method, self.get_pkg_name(pkg))]
        if not to_uninstall:
        ...
        for idx, pkg in enumerate(to_uninstall, start=1):
            ...
            if not self.is_package_installed(self.selected_method, pkg_name):
                stdscr.addstr(log_row, 2, f"Already uninstalled: {pkg_name}",
                ...
                if cmd:
                    self.uninstall_package_cmd(cmd)
                    PackageInstallerApp.is_package_installed.cache_clear()
                stdscr.addstr(log_row, 2, f"Uninstalled: {pkg_name}",
                              curses.color_pair(3) | curses.A_BOLD)
            stdscr.refresh()
        stdscr.addstr(7 + total, 2, self.category_uninstall_complete_text, curses.A_REVERSE)
        stdscr.refresh()
        stdscr.getch()
    ...
    def install_selected_packages(self, stdscr: curses.window) -> None:
        ...
        for idx, pkg in enumerate(self.selected_packages, start=1):
            ...
            if self.is_package_installed(self.selected_method, pkg):
                stdscr.addstr(log_row, 2, f"Already installed {self.CATEGORY_MARKER_INSTALLED}: {pkg}",
                              curses.color_pair(2) | curses.A_BOLD)
            else:
        ...
        while True:
            ...
            for idx in range(scroll_offset, min(len(menu_items), scroll_offset + max_items)):
                ...
                if isinstance(item, tuple):
                    ...
                    if item_type in ("install", "back"):
                        ...
                        if self.is_package_installed(self.selected_method, pkg_name):
                            display_str = f"{self.CATEGORY_MARKER_INSTALLED} {base_text}"
                    ...
                    display_str = "> " + item if idx == current_selection else "  " + item
                    attr = curses.A_NORMAL
                    if mode in ("methods", "packages") and (isinstance(item, str) and item == self.back_button_text):
                        attr = curses.color_pair(5)
                if idx == current_selection:
                    attr |= curses.A_REVERSE
                try:
                    stdscr.addstr(self.MENU_START_ROW + idx - scroll_offset, 4, display_str, attr)
                except curses.error:
                    pass
            ...
            if k == curses.KEY_MOUSE:
                try:
                    ...
                    continue
                if self.MENU_START_ROW <= my < self.MENU_START_ROW + min(len(menu_items) - scroll_offset, max_items):
            ...
            if k == ord("q"):
                ...
                if mode == "distros":
                    ...
                    if isinstance(current_item, tuple) and current_item[2] == "install":
                        ...
                        current_selection = 0
                        scroll_offset = 0
                    elif isinstance(current_item, tuple) and current_item[2] == "package":
                ...
                if mode == "packages":
                    ...
                    remaining = [self.get_pkg_name(pkg) for pkg in cat_pkgs
                                 if not self.is_package_installed(self.selected_method, self.get_pkg_name(pkg))]
                    if remaining and all(pkg in self.selected_packages for pkg in remaining):
